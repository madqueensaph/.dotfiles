...
globalkeys = gears.table.join(
	...
	awful.key({ modkey, "Shift", "Control" }, "Delete",	-- Lock Screen
		function()
			awful.spawn("sh " .. binDir .. "lock.sh")
		end ),
	awful.key({ modkey, "Shift", "Control" }, "End",	-- Lock Screen
		function()
			awful.spawn("sh " .. binDir .. "lock-no-mute.sh")
		end ),
	awful.key({ modkey }, "Print",	-- Take Screenshot of Focused Window
		function()
			awful.spawn.easy_async_with_shell("maim -unsm 1 | xclip -selection clipboard -t image/png")
		end ),
	...
	awful.key({ modkey, "Control", "Shift" }, "Print",	-- Captures main laptop screen
		function()
			awful.spawn.with_shell("maim -unm 5 -g 1600x900+1920+0 $HOME/Pictures/Screenshots/$(date +'%Y-%m-%d_%H-%M-%S').png")
		end ),
...
awful.rules.rules = {
	...
	{	rule_any = { class = { "Xfce4-terminal", "Emacs" } },
